<?php 
require_once('clases/ListaEmpleados.php');
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['codigo']) && isset($_POST['nombre']) && isset($_POST['apellido']) && isset($_POST['sexo']) &&isset($_POST['sueldo'])) {
    ...
    if(isset($_POST['guardar'])) {
        ...
        if ($listaEmpleados->consultarEmpleado($codigo) !== null) {
            $respuesta_consulta = "<div class='mensaje-error'>El empleado con el código $codigo ya está registrado.</div>";
        }else{
            $empleado_guardar = $listaEmpleados->guardarEmpleado($codigo, $nombre, $apellido, $sexo, $sueldo);
            $respuesta_consulta = "<div class='mensaje-exitoso'>Guardado correctamente.</div>";
        }        
    } elseif (isset($_POST['adelante_atras'])) {
        $_SESSION['mostrar_adelante'] = true;
    } elseif (isset($_POST['atras_adelante'])) {
        $_SESSION['mostrar_adelante'] = false;
    }elseif(isset($_POST['consultar'])) {
        // Obtener el código del empleado a consultar
        $codigo_consulta = $_POST['codigo'];
        // Consultar al empleado
        $empleado_consultado = $listaEmpleados->consultarEmpleado($codigo_consulta);
        ...
        if ($empleado_consultado !== null) {
            ...
            $respuesta_sexo = $empleado_consultado->sexo;
        ...
        $sexo_actualizar = $_POST['sexo'];
        $sueldo_actualizar = $_POST['sueldo'];
        // Actualizar al empleado
        $resultado_actualizacion = $listaEmpleados->actualizarEmpleado($codigo_actualizar, $nombre_actualizar, $apellido_actualizar, $sexo_actualizar, $sueldo_actualizar);
        ...
        $resultado_eliminacion = $listaEmpleados->eliminarEmpleado($codigo_eliminar);
